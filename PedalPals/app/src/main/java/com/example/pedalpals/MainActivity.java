...
public class MainActivity extends AppCompatActivity {
    ...
    private void handleUserLoginWithThread() {
        userlogin.setOnClickListener(v -> {
            new Thread(() -> {
                try {
                    Thread.sleep(1000); // Simulated delay
                    preloadUserData(); // Custom method to preload data
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }

                runOnUiThread(() -> {
                    Toast.makeText(MainActivity.this, "User data preloaded. Navigating to User Login...", Toast.LENGTH_SHORT).show();
                    Intent i = new Intent(MainActivity.this, UserLogin.class);
                    startActivity(i);
                });
            }).start();
